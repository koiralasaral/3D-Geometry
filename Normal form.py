...
plt.show()
# ----- Normal form: lx + my + nz = 0 -----
# Here we choose a normal vector for example:
l, m, n = 1, -1, 2

# Solve for z: z = (-l*x - m*y) / n (assuming n != 0)
Z_norm = (-l * X - m * Y) / n

fig = plt.figure(figsize=(8, 6))
ax = fig.add_subplot(111, projection='3d')
normal_plot = ax.plot_surface(X, Y, Z_norm, alpha=0.6, cmap="coolwarm", edgecolor='none')

ax.set_xlabel("X-axis")
ax.set_ylabel("Y-axis")
ax.set_zlabel("Z-axis")
ax.set_title("Normal Form Plane: x - y + 2z = 0")
plt.colorbar(normal_plot, shrink=0.5, aspect=5, label="Plane Surface Value")
plt.show()
for i in range(1, 6):
    # Varying coefficients with the loop index i.
    # For demonstration, we use:
    a_new = i
    b_new = -i
    c_new = i + 1
    d_new = -i  # shifting the plane along the normal direction
    
    # Calculate Z for the new plane: ax + by + cz + d = 0  --->  Z = (-a*x - b*y - d)/c
    ...
    ax.set_ylabel("Y-axis")
    ax.set_zlabel("Z-axis")
    ax.set_title(f"Plane for i = {i}: {a_new}x + ({b_new})y + {c_new}z + ({d_new}) = 0")
